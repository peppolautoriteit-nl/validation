...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron" is-a="PEPPOL12-T10" id="PEPPOL12-UBL-T10">
  ...
  <param name="EUGEN-T10-R042" value="((cbc:TaxAmount) and (cbc:TaxableAmount) and (cac:TaxCategory/cbc:Percent) and (xs:decimal(cbc:TaxAmount - 1) &lt; xs:decimal(cbc:TaxableAmount * (xs:decimal(cac:TaxCategory/cbc:Percent) div 100))) and (xs:decimal(cbc:TaxAmount + 1) &gt; xs:decimal(cbc:TaxableAmount * (xs:decimal(cac:TaxCategory/cbc:Percent) div 100)))) or not(cac:TaxCategory/cbc:Percent) or not(cbc:TaxableAmount)"/>
